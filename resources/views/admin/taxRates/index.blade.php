...
<div class="max-w-7xl mx-auto py-6">
    <div class="bg-white shadow-lg rounded-2xl p-6">
                ...
                @can('tax_rate_create')
                    <a href="{{ route('admin.tax-rates.create') }}" 
                       class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg shadow">
                        <i class="fas fa-plus mr-1"></i> {{ trans('global.add') }} {{ trans('cruds.taxRate.title_singular') }}
                    </a>
                    ...
                    @include('csvImport.modal', [
                        'model' => 'TaxRate', 
                        'route' => 'admin.tax-rates.parseCsvImport'
                    ])
                @endcan
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 datatable datatable-TaxRate">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($taxRates as $key => $taxRate)
                        <tr data-entry-id="{{ $taxRate->id }}" class="hover:bg-gray-50">
                            <td class="px-4 py-3"></td>
                            <td class="px-4 py-3 text-sm text-gray-700">{{ $taxRate->id ?? '' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-700">{{ $taxRate->name ?? '' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-700">{{ $taxRate->parcentage ?? '' }}</td>
                            <td class="px-4 py-3 text-center space-x-1">
                                @can('tax_rate_show')
                                    <a href="{{ route('admin.tax-rates.show', $taxRate->id) }}" 
                                ...
                                @endcan
                                @can('tax_rate_delete')
                                    <form action="{{ route('admin.tax-rates.destroy', $taxRate->id) }}" method="POST" 
...
<script>
    $(function () {
        ...
        // Custom search bar
        $('#taxRateSearch').on('keyup change clear', function () {
