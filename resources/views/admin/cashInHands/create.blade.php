        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.cashInHand.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.cash-in-hands.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('adjustment') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="adjustment" id="adjustment" required>
                                ...
                                @foreach(App\Models\CashInHand::ADJUSTMENT_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('adjustment', '--select--') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('enter_amount') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="enter_amount" id="enter_amount" value="{{ old('enter_amount', '') }}" step="0.01" required>
