                        ...
                        <tr data-entry-id="{{ $saleInvoice->id }}" class="hover:bg-gray-50">
                            ...
                            <td class="px-4 py-3 text-center space-x-1">
                                @can('sale_invoice_show')
                                    <a href="{{ route('admin.sale-invoices.show', $saleInvoice->id) }}" class="px-2 py-1 bg-blue-600 text-white rounded text-xs hover:bg-blue-700">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                @endcan
                                @can('sale_invoice_edit')
                                    <a href="{{ route('admin.sale-invoices.edit', $saleInvoice->id) }}" class="px-2 py-1 bg-indigo-600 text-white rounded text-xs hover:bg-indigo-700">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                @endcan
                                @can('sale_invoice_delete')
                                    <form action="{{ route('admin.sale-invoices.destroy', $saleInvoice->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" class="inline-block">
                                        @method('DELETE')
                                        @csrf
