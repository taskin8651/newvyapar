...
<div class="p-6 bg-gray-100 min-h-screen">
    <div class="max-w-4xl mx-auto">
        <div class="bg-white shadow rounded-lg p-6">
            ...
            <form method="POST" action="{{ route('admin.ledgers.update', $ledger->id) }}" class="space-y-6">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <select name="expense_category_id" id="expense_category_id" required
                        ...
                        @foreach($expenseCategories as $id => $name)
                            <option value="{{ $id }}" {{ old('expense_category_id', $ledger->expense_category_id) == $id ? 'selected' : '' }}>
                                {{ $name }}
                            </option>
                        @endforeach
                    </select>
