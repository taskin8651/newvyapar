...
<div class="max-w-7xl mx-auto py-6">
    <div class="bg-white shadow-lg rounded-2xl p-6">
                ...
                @can('unit_create')
                    <a href="{{ route('admin.units.create') }}" 
                       class="px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded-lg shadow">
                        <i class="fas fa-plus mr-1"></i> {{ trans('global.add') }} {{ trans('cruds.unit.title_singular') }}
                    </a>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 datatable datatable-Unit">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    @foreach($units as $key => $unit)
                        <tr data-entry-id="{{ $unit->id }}" class="hover:bg-gray-50">
                            <td class="px-4 py-3"></td>
                            <td class="px-4 py-3 text-sm text-gray-700">{{ $unit->id ?? '' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-700">{{ $unit->base_unit ?? '' }}</td>
                            <td class="px-4 py-3 text-sm text-gray-700">{{ $unit->secondary_unit ?? '' }}</td>
...
<script>
    $(function () {
        ...
        // Custom search bar
        $('#unitSearch').on('keyup change clear', function () {
