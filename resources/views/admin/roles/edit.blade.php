    ...
    <div class="max-w-4xl mx-auto bg-white shadow-lg rounded-2xl border border-gray-100">
        ...
        <div class="px-6 py-4 border-b border-gray-200 flex items-center justify-between">
            <h2 class="text-xl font-semibold text-gray-800 flex items-center space-x-2">
                ...
                <span>{{ trans('global.edit') }} {{ trans('cruds.role.title_singular') }}</span>
            </h2>
        ...
        <div class="p-6" x-data="permissionSelect()">
            <form method="POST" action="{{ route('admin.roles.update', [$role->id]) }}" enctype="multipart/form-data" class="space-y-8">
                @method('PUT')
                @csrf
...
<script>
    function permissionSelect() {
        return {
            // Pre-select old values OR role->permissions
            selected: @json(old('permissions', $role->permissions->pluck('id')->toArray())),
