            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-blue-50 p-4 rounded-lg shadow-inner">
                    ...
                    <input type="text" name="company_name" id="company_name" 
                           value="{{ old('company_name', $addBusiness->company_name) }}" required
                ...
                <div class="bg-blue-50 p-4 rounded-lg shadow-inner">
                    ...
                    <input type="text" name="legal_name" id="legal_name" 
                           value="{{ old('legal_name', $addBusiness->legal_name) }}" required
                           class="w-full rounded-lg border-gray-300 shadow-sm px-3 py-2 text-sm focus:border-indigo-500 focus:ring-indigo-500">
                    @error('legal_name')
                        <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div class="bg-green-50 p-4 rounded-lg shadow-inner">
                    ...
                    <select name="business_type" id="business_type"
                        ...
                        @foreach(App\Models\AddBusiness::BUSINESS_TYPE_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('business_type', $addBusiness->business_type) == $key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="bg-green-50 p-4 rounded-lg shadow-inner">
                    ...
                    <select name="industry_type" id="industry_type"
                        ...
                        @foreach(App\Models\AddBusiness::INDUSTRY_TYPE_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('industry_type', $addBusiness->industry_type) == $key ? 'selected' : '' }}>
                                {{ $label }}
                            </option>
                        @endforeach
                    </select>
