...
<head>
    <meta charset="UTF-8">
    <title>Tax Invoice - {{ $currentStock->id }}</title>
...
<body class="bg-gradient-to-br from-blue-50 to-indigo-100 min-h-screen py-4 px-2 text-xs">
        ...
        <div class="invoice-container bg-white rounded-xl overflow-hidden text-xs">
            ...
            <div class="header-gradient text-white p-4 text-xs">
                <div class="flex justify-between items-start">
                    <div>
                        <h1 class="text-lg font-bold">TAX INVOICE</h1>
            ...
            <div class="p-4 border-b border-gray-200 text-xs">
                <h3 class="text-sm font-semibold text-blue-700 mb-1 flex items-center">
                    <i class="fas fa-user-circle mr-1"></i> Bill To
                </h3>
                <div class="bg-blue-50 p-2 rounded-lg border border-blue-200 text-xs">
                    @if($currentStock->party)
                        <h4 class="font-medium text-blue-800">{{ $currentStock->party->party_name }}</h4>
                        <p class="text-blue-700">{{ $currentStock->party->billing_address }}</p>
                        <p class="text-blue-700">Contact No.: {{ $currentStock->party->phone_number }}</p>
                        <p class="text-blue-700">State: {{ $currentStock->party->state }}</p>
                    @else
                        <p class="italic text-gray-500">No party selected</p>
                    @endif
                </div>
            </div>
                ...
                <table class="w-full text-xs border border-gray-200">
                    <thead>
                        <tr class="table-header text-white text-xs">
                            ...
                            <th class="p-1 text-left">MRP</th>
                            <th class="p-1 text-left">Quantity</th>
                    ...
                    <tbody>
                        @php $total = 0; @endphp
                        @forelse($currentStock->items as $key => $item)
                            @php
                            ...
                            <tr class="border-b">
                                ...
                                <td class="p-1">Rs {{ number_format($item->sale_price,2) }}</td>
                                <td class="p-1">{{ $qty }}</td>
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="text-center p-1 italic text-gray-500">No items found</td>
                            </tr>
                        @endforelse
                        <tr class="bg-blue-50 font-semibold">
                            <td class="p-1" colspan="4">Total</td>
                            <td class="p-1">{{ $currentStock->items->sum(fn($i) => $i->pivot->quantity ?? 1) }}</td>
            ...
            <!-- Invoice Details + Amount in Words Table -->
            <div class="p-4 border-b border-gray-200 overflow-x-auto text-xs">
                                ...
                                <thead>
                                    <tr>
                                        <th colspan="2" class="text-center text-blue-700 font-semibold py-1 text-sm">
                                            <i class="fas fa-file-invoice mr-1"></i> Invoice Details
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        ...
                                        <td class="text-right px-2">{{ $currentStock->id }}</td>
                                    </tr>
                                    <tr>
                                        <td class="font-medium py-1 px-2">Date:</td>
                                        <td class="text-right px-2">{{ optional($currentStock->created_at)->format('d-m-Y') }}</td>
                                    </tr>
                                    <tr>
                                        <td class="font-medium py-1 px-2">Time:</td>
                                        <td class="text-right px-2">{{ optional($currentStock->created_at)->format('h:i A') }}</td>
                                    </tr>
                                    <tr>
                                        <td class="font-medium py-1 px-2">Place of Supply:</td>
                                        <td class="text-right px-2">{{ $currentStock->party->state ?? '-' }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </td>
                        ...
                        <td class="align-top w-1/2 pl-2">
                            <table class="w-full border rounded-lg border-blue-200 bg-blue-50 text-xs">
                                <thead>
                                    <tr>
                                        <th colspan="2" class="text-center text-blue-700 font-semibold py-1 text-sm">
                                            <i class="fas fa-receipt mr-1"></i> Invoice Amount in Words
                                        </th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <tr>
                                        <td colspan="2" class="bg-blue-100 font-medium py-1 px-2 rounded mb-1">
                                            {{ \App\Helpers\NumberHelper::toWords($total) }}
                                        </td>
                                    </tr>
                                    <tr>
                                        <td colspan="2" class="py-1 px-2 text-left">
                                            <span class="bg-green-100 text-green-800 px-1 py-0.5 rounded-full inline-flex items-center text-xs">
            ...
            <!-- Transaction Summary + Bank Details Table -->
            <div class="p-4 border-b border-gray-200 overflow-x-auto text-xs">
                <table class="w-full text-xs border border-gray-200 rounded-lg">
                    <thead>
                        <tr class="bg-blue-100 text-blue-700 font-semibold text-xs text-center">
                            <th class="p-2 w-1/2">Transaction Summary</th>
                            <th class="p-2 w-1/2">Bank Details</th>
                        </tr>
                    </thead>
                    <tbody>
                            ...
                            <td class="p-2 align-top">
                                <table class="w-full text-xs">
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>Received</td>
                                            <td class="text-right text-green-600">Rs 0.00</td>
                                        </tr>
                                        <tr class="bg-gray-50">
                                            <td>Previous Balance</td>
                                            <td class="text-right">Rs {{ number_format($total,2) }}</td>
                                        </tr>
                                        <tr class="font-semibold border-t pt-1">
                                            <td>Current Balance</td>
                                            <td class="text-right">Rs {{ number_format($total*2,2) }}</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </td>
                                ...
                                <table class="w-full text-xs">
                                    <tbody>
                                        <tr>
                                            ...
                                            <td>HDFC BANK, KADAMKUAN</td>
                                        </tr>
                                        <tr class="bg-gray-50">
                                            <td class="font-medium">Account No.:</td>
                                            <td>50200098062030</td>
                                        </tr>
                                        <tr>
                                            <td class="font-medium">IFSC Code:</td>
                                            <td>HDFC0009296</td>
                                        </tr>
                                        <tr class="bg-gray-50">
                                            <td class="font-medium">Account Holder:</td>
                                            <td>MARUTI SUZUKI VENTURES</td>
                                        </tr>
                                    </tbody>
                                </table>
                            </td>
            ...
            <!-- Terms + Authorized Signatory Table -->
            <div class="p-4 border-b border-gray-200 overflow-x-auto text-xs">
                <table class="w-full text-xs border border-gray-200 rounded-lg">
                    <tbody>
                        <tr class="align-top">
                            ...
                            <td class="p-2 w-1/2 border-r border-gray-200">
                                <h3 class="font-semibold text-blue-700 mb-1 flex items-center text-xs">
                                    <i class="fas fa-file-contract mr-1"></i> Terms and Conditions
                                </h3>
                                <p class="text-blue-600 mb-1 text-xs">Thanks for doing business with us!</p>
                                <div class="bg-yellow-50 p-2 rounded border text-xs">
                                    <h4 class="font-semibold text-orange-700 mb-1 text-xs">RETURN POLICY:</h4>
                                    <p class="text-orange-800 text-xs">
                                        Sold products can be returned within 3 days from invoice date. After that products will not be returned.
                                    </p>
                                </div>
                            </td>
                            <!-- Authorized Signatory Column -->
                          <td class="p-2 w-1/2 text-center align-top">
    <div class="flex flex-col justify-end h-full" style="height: 120px;"> <!-- Set a fixed height -->
        ...
        <p class="text-gray-500 text-xs">Your trusted automotive partner</p>
    </div>
</td>
    ...
    <p class="mt-1">
        Invoice generated on: {{ now()->format('d-m-Y \a\t h:i A') }} by 
